...
RUN apk add --no-cache mariadb-connector-c postgresql-client libcurl py3-gunicorn py3-pyldap py3-flask py3-psycopg2 xmlsec tzdata libcap && \
    addgroup -S ${USER} && \
    ...
    apk update && apk add --no-cache pdns pdns-backend-sqlite3 && chown -R pdns:pdns /etc/pdns && \
    sed -i '3d' /etc/apk/repositories && \
    sed -i '4d' /etc/apk/repositories
    
COPY --from=builder /usr/bin/flask /usr/bin/
COPY --from=builder /usr/lib/python3.10/site-packages /usr/lib/python3.10/site-packages/
COPY --from=builder --chown=root:${USER} /app /app/
COPY ./pdns/pdns.conf /data/
COPY ./pdns/pdns.sqlite3 /data/
COPY ./docker/entrypoint.sh /usr/bin/

WORKDIR /app
RUN mkdir /var/run/pdns && chown ${USER}:${USER} ./configs /app /var/run/pdns && \
